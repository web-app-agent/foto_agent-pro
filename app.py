"""
FotoAgent Pro - Streamlit App (UI Moderna, Elegante e Estilo SaaS)
Este aplicativo implementa uma interface intuitiva, profissional e organizada,
com suporte a Geração Direta, Composição Multi-Imagem, Battle Mode (Side-by-Side) e Chat de Ideação.
"""
...
# Estilização CSS Customizada (Aesthetic Dark Mode SaaS sem falhas)
st.markdown("""
    <style>
    /* Fundo Geral */
    .stApp {
    ...
    /* Cartões e Blocos */
    .css-1r6slb0, .stTabs [data-baseweb="tab-panel"] {
...
st.markdown("""
    <div class="hero-container">
        ...
        <p style="color: #94a3b8; font-size: 14px; margin: 0;">Plataforma Profissional de Ensaios Fotográficos, Composição Multi-Imagem & Battle Mode (Side-by-Side)</p>
    </div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    with col_left:
        ...
        existing_prompt = st.text_area(
            ...
            placeholder="Cole seu prompt pronto aqui (ex: Professional high-end photography of...)",
            height=100
        )
        
        st.markdown("---")
        st.markdown("### 🖼️ Composição Multi-Imagem (Blending)")
        st.markdown("<p style='font-size: 12px; color: #94a3b8; margin-top: -10px;'>Insira um fundo e um sujeito para mesclagem fotorrealista.</p>", unsafe_allow_html=True)
    ...
    with col_right:
        ...
        st.markdown("<p style='font-size: 12px; color: #94a3b8; margin-top: -10px;'>Compare duas opções de IAs distintas e escolha a melhor.</p>", unsafe_allow_html=True)
        
        if st.session_state.get('generated', False):
            res_col1, res_col2 = st.columns(2)
            
            with res_col1:
                st.markdown("<div style='background: #0b0f19; padding: 12px; border-radius: 12px; border: 1px solid #1e293b; text-align: center;'>", unsafe_allow_html=True)
            ...
            st.markdown("""
                <div style='background: #0b0f19; border: 2px dashed #1e293b; border-radius: 1rem; height: 350px; display: flex; flex-direction: column; align-items: center; justify-content: center; color: #64748b; text-align: center; padding: 20px;'>
...
with tab2:
    ...
    st.markdown("<p style='font-size: 13px; color: #94a3b8;'>Converse com o agente para estruturar conceitos, definir iluminação e gerar prompts profissionais do zero.</p>", unsafe_allow_html=True)
